...
use chrono::{DateTime, Datelike, Local, NaiveDate, Utc, FixedOffset};
...
impl BanglaDate {
    ...
    pub fn from_date_with_region(current_date: NaiveDate, region: CalendarRegion) -> Self {
        ...
        let bangla_year = if is_before_pohela { g_year - 594 } else { g_year - 593 };
    ...
    pub fn format_bangla(&self, use_bangla_numerals: bool) -> String {
        ...
        format!("{}{} {}, {} বঙ্গাব্দ", day_str, suffix, self.get_month_name(), year_str)
    }
}
...
pub fn format_gregorian_date(dt: DateTime<Local>, use_bangla_names: bool, use_bangla_numerals: bool) -> String {
    let day = dt.day();
    ...
    let suffix = if use_bangla_names { " ঈসায়ী" } else { "" };
...
mod tests {
    ...
    fn tz_usa_est() -> FixedOffset { tz(-5, 0) }      // EST (New York)
    fn tz_usa_pst() -> FixedOffset { tz(-8, 0) }      // PST (Los Angeles)
    fn tz_uk() -> FixedOffset { tz(0, 0) }            // GMT (London)
    fn tz_india() -> FixedOffset { tz(5, 30) }        // IST (India)
    fn tz_bangladesh() -> FixedOffset { tz(6, 0) }    // BST (Bangladesh)
    fn tz_china() -> FixedOffset { tz(8, 0) }         // CST (China)
    fn tz_russia_msk() -> FixedOffset { tz(3, 0) }    // MSK (Moscow)
    fn tz_canada_est() -> FixedOffset { tz(-5, 0) }   // EST (Toronto)
    ...
    fn test_timezone_conversion_morning() {
        ...
        for (country, local_tz, tz_name) in test_cases {
            ...
            println!("   📅 Bangladesh Calendar: {} {} {}", bd_bangla.day, bd_bangla.get_month_name(), bd_bangla.year);
            println!("   📅 India Calendar:      {} {} {}", in_bangla.day, in_bangla.get_month_name(), in_bangla.year);
            println!();
        }
    }
    ...
    fn test_date_boundary_crossing() {
        ...
        let ny_time = tz_usa_est().with_ymd_and_hms(2026, 1, 25, 23, 0, 0).unwrap();
        let utc_time = ny_time.with_timezone(&Utc);
        let bd_time = utc_time.with_timezone(&tz_bangladesh());
        
        let bd_bangla = BanglaDate::from_utc_with_region(utc_time, CalendarRegion::Bangladesh);
        
        println!("🗽 New York: {} (Jan 25, 11:00 PM)", ny_time.format("%Y-%m-%d %H:%M"));
        println!("🌍 UTC:      {}", utc_time.format("%Y-%m-%d %H:%M"));
        println!("🇧🇩 Bangladesh: {} (Next day!)", bd_time.format("%Y-%m-%d %H:%M"));
        println!("📅 Bangla Date: {}ই {} {}", bd_bangla.day, bd_bangla.get_month_name(), bd_bangla.year);
    ...
    fn test_january_26_2026_bangladesh() {
        ...
        println!("✓ January 26, 2026 (Bangladesh) = {}ই {} {} বঙ্গাব্দ", 
                 bangla.day, bangla.get_month_name(), bangla.year);
    }
    ...
    fn test_january_26_2026_india() {
        ...
        println!("✓ January 26, 2026 (India) = {}ই {} {} বঙ্গাব্দ", 
                 bangla.day, bangla.get_month_name(), bangla.year);
    }
    ...
    fn test_world_clock_utc_midnight() {
        ...
        for (country, local_tz, calendar) in regions {
            ...
            println!("   📅 Bangla: {}ই {}, {} বঙ্গাব্দ", bangla.day, bangla.get_month_name(), bangla.year);
            println!();
        }
    }
}
